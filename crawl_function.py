...
for func_info in data:
    try:
        ...
        func_prototype = func_info.split('\t')[2]
        print(func_prototype)
        param_list = func_prototype.split('(')[1].split(')')[0]
        ...
        for parameter in param_list:
            ...
            type_str = ''
            type_flag = False
            for param_i in param_split:
                if param_i == 'const':
                    continue
                type_str += param_i
                break
            print(type_str, idx)
            temp_dict[str(idx)] = type_str
            idx+=1
        print('--------------------------------')
        json_data[func_name] = temp_dict
    except:
        continue

f = open('crawl_function.json', 'w')
